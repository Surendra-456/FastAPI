...
from app.schemas import PostCreate,PostUpdate,PostResponse,PaginatedPostsResponse
...
#ge=1 means greater or equal 1 and le means lesstha or equal to
...
async def update_post_full(post_id: int,currentUser: CurrentUser,post_data:PostCreate,db: Annotated[AsyncSession, Depends(get_db)],):
    result = await db.execute(select(models.Post).options(selectinload(models.Post.author)).where(models.Post.id == post_id))
...
async def update_post_partial(post_id: int,currentUser: CurrentUser,post_data:PostUpdate,db: Annotated[AsyncSession, Depends(get_db)],):
    result = await db.execute(select(models.Post).options(selectinload(models.Post.author)).where(models.Post.id == post_id))
...
async def delete_post(post_id: int,currentUser: CurrentUser, db: Annotated[AsyncSession, Depends(get_db)],):
    result = await db.execute(select(models.Post).options(selectinload(models.Post.author)).where(models.Post.id == post_id and models.User.id==currentUser.id ))
...
async def create_post(post: PostCreate,currentUser: CurrentUser,db: Annotated[AsyncSession, Depends(get_db)],):
    ...
    new_post = models.Post(title=post.title,content=post.content,user_id=currentUser.id,)
